"""LeadCreditEngineOutput adapter — internal credit decisioning output.
...
class LeadCreditEngineAdapter(EnrichmentAdapter):
    source_name = "lead_credit_engine"
    ...
    def _mock_matched_payload(rec: CanonicalRecord, rng: random.Random) -> dict[str, Any]:
        return {
            "hit": True,
            "decision": rng.choice(["approved", "declined", "manual_review"]),
            "credit_limit": round(rng.uniform(0, 300_000), 2),
            "risk_tier": rng.choice(["low", "medium", "high"]),
            "internal_score": round(rng.uniform(0, 1), 3),
        }
